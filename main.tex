...
\subsection{Regular expression to \(\varepsilon\)-NFA}\label{sec:regexp-to-enfa}

In the figures: ,,start'' and ,,end'' state are initial
and terminating.
\textit{White} dot is an initial state, \textit{black} dot is terminating state.
\begin{enumerate}
  \item if regular expression is empty symbol, then create singleton NFA.
  \item if regular expression is literal character,
    then create the NFA with two nodes and add transition between these nodes
    by the character.
...
\begin{theorem}[Kleene's theorem]
  A language is accepted by regular expression
  \textit{iff} the language is accepted by finite automata.
\end{theorem}
...
\begin{enumerate}
  \item (Optional) Make ,,transitive closure'' of \(\varepsilon\)-transitions
    (see figure \ref{fig:enfa-to-nfa-1})
  \begin{figure}[!htbp]
    \centering
    \begin{tikzpicture}[node distance=2cm]
      ...
      \draw[->] (B) to node[below] {\(\varepsilon\)} (C);
      \draw[->] (A) to[out=-30, in=-150] node[below] {\(\varepsilon\)} (C);
    \end{tikzpicture}
    \caption{Make ,,transitive closure'' of \(\varepsilon\)-transitions}%
  ...
  \end{figure}
  \item Perform symbol-transition over \(\varepsilon\)-transitions
    (see figure \ref{fig:enfa-to-nfa-3})
  \begin{figure}[!htbp]
    \centering
    \begin{tikzpicture}[node distance=2cm]
      \node[draw, circle] (A) at (0, 0) {\(A\)};
